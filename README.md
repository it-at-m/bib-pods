...
- **Standortdaten**: `npm run extract-msb-standorte` in `bib-src` aktualisiert `definitions/msb-standorte.ttl` aus den aufklappbaren Einträgen unter [Unsere Standorte](https://www.muenchner-stadtbibliothek.de/orte-zeiten): Namen, verfügbare Adressen und „Mehr“-Links. Nominatim ergänzt Koordinaten bei jedem Lauf neu, einzeln mit Abstand zwischen den Anfragen. `NOMINATIM_ENDPOINT` erlaubt eine andere Instanz.
